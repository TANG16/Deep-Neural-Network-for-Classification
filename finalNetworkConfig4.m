...
function finalNetwork = finalNetworkConfig3(autoencHid1,...
    ...
    % Connect the 5th layer to the 3nd layer
    finalNetwork.layerConnect(5,4) = 1;
    
    % Connect the output to the 5th layer
    finalNetwork.outputConnect(1,5) = 1;

    % Add a connection for a bias term for each layer
    finalNetwork.biasConnect = [1; 1; 1; 1; 1];
    ...
    % Set the size of the second layer to the same as the layer in autoencHid2
    finalNetwork.layers{4}.size = hiddenSize4;
    
    % Set the size of the third layer to the same as the layer in finalSoftmax
    finalNetwork.layers{5}.size = outputSizeFinal;
    ...
    finalNetwork.layers{4}.transferFcn = 'logsig';

    % Set the transfer function for the third layer to the same as in
    % finalSoftmax
    finalNetwork.layers{5}.transferFcn = 'softmax';
    ...
    finalNetwork.LW{4,3} = autoencHid4.IW{1,1};
    finalNetwork.b{4} = autoencHid4.b{1,1};
    finalNetwork.LW{5,4} = finalSoftmax.IW{1,1};
    finalNetwork.b{5} = finalSoftmax.b{1,1};
