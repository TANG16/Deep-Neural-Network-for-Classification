...
%% Parameters
epochs_auto1 = 500;   %400
epochs_auto2 = 500;   %100 
epochs_auto3 = 500;   %100 
epochs_auto4 = 500;   %100 
epochs_softmax = 500; %400
epochs_final = 500;   %400

% hideen layers outputs
outputSize1 = 500; % reduction size (number of neurons in hidden layer)
outputSize2 = 250;% reduction size (number of neurons in hidden layer)
outputSize3 = 100;% reduction size (number of neurons in hidden layer)
outputSize4 = 50;% reduction size (number of neurons in hidden layer)
...
%% Training the forth Autoencoder
%  Train autoencoder on the features generated from the prev autoencoder.
% ========================================================================
autoenc4 = autoencoder(features3,outputSize4,epochs_auto4); % create and train autoencoder
%view(autoenc3);

% Create second hidden layer uses weights and bias trained in autencoder2
autoencHid4 = autoencoderHid(autoenc4,outputSize3,outputSize4);
%view(autoencHid3);

% Generate second features
features4 = autoencHid4(features3);
...
finalSoftmax = softmaxLayer(outputSize4,outputSizeFinal,epochs_softmax);

% train the softmax layer in a supervised fashion
% using labels for the training data.
finalSoftmax = train(finalSoftmax,features4,tTrain);
%view(finalSoftmax);
